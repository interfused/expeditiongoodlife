...
function pm_one_click_before_insert_post( $post, $one_click ) {

    if (!class_exists('simple_html_dom')) {
        require_once PM_THEME_DIR . 'vendor/interfused/interfused-one-click/inc/simple_html_dom.php';
    }
    ...
    $gallery_shortcode = pm_get_content_shortcode($post_object, 'gallery');
    ...
    if (!empty($post_object->post_content)) {
        ...
        $post_object->post_content = $one_click->replace_shortcode_attachment_id($post_object->post_content, 'staff_featured', 'member', 'pm_staff');
...
add_filter( 'pm_one_click_before_insert_post', 'pm_one_click_before_insert_post', 10, 2 );
 ...
 * @author
 **/
function pm_one_click_before_wp_update_nav_menu_item($new_menu_item, $menu_item, $one_click)
{
    switch ($menu_item['type']) {
        ...
        case 'taxonomy':
            switch($menu_item['object']) {
                case 'pm_mega_menu':
                    $mega_menu = get_page_by_title('Mega Menu', 'OBJECT', 'pm_mega_menu');
                    $new_menu_item['menu-item-object-id'] = $mega_menu->ID;
                    break;
                case 'pm_mega_columns':
...
add_filter('pm_one_click_before_wp_update_nav_menu_item', 'pm_one_click_before_wp_update_nav_menu_item', 10, 3);

function pm_filter_import_packages( $packages ) {
    return array(
        array(
            'id'           => THEME_SHORT . '-main-demo',
            'name'         => __('Main Demo Content', 'my_theme-admin-td'),
            ...
            'description'  => __('Main demo content, many pages for you to use for every kind of site.', 'my_theme-admin-td'),
            'type'         => 'interfused',
            'importUrl'    => 'https://one-click-import.s3.amazonaws.com/omega/',
            'importFile'   => 'import.json',
            'requirements' => array(
                array(
                    'name' => __('Visual Composer Plugin', 'my_theme-admin-td'),
                ...
                array(
                    'name' => __('Revolution Slider', 'my_theme-admin-td'),
        ...
        array(
            ...
            'name'         => __('WooCommerce Shop Content', 'my_theme-admin-td'),
            ...
            'description'  => __('Installs all the woocommerce products and shop pages that you see on the demo site.', 'my_theme-admin-td'),
            'type'         => 'interfused',
            'importUrl'    => 'https://one-click-import.s3.amazonaws.com/omega/',
            ...
            'requirements' => array(
                ...
                array(
                    'name' => __('Woo Commerce Plugin', 'my_theme-admin-td'),
...
add_filter( 'pm_one_click_import_packages', 'pm_filter_import_packages', 10, 1 );

function pm_one_click_export_slideshows( $package_file ) {
    switch( $package_file ) {
        case 'import.json':
            $slideshows = array(
                array(
...
add_filter( 'pm_one_click_export_slideshows', 'pm_one_click_export_slideshows' );


function pm_one_click_import_download_url( $filename ) {
    return 'http://one-click-import.s3.amazonaws.com/omega/images/' . $filename;
}
add_filter( 'pm_one_click_import_download_url', 'pm_one_click_import_download_url', 10, 1 );

function pm_one_clicl_theme_docs_url( $url ) {
    return 'http://omegadocs.oxygenna.com';
}
add_filter( 'pm_one_clicl_theme_docs_url', 'pm_one_clicl_theme_docs_url', 10, 1 );
 ...
 * @author
 **/
function pm_one_click_import_add_metadata_menu_item($new_menu_item_id, $menu_item, $one_click)
{
    // add custom data if exists
    if (isset($menu_item['custom_fields'])) {
        foreach ($menu_item['custom_fields'] as $key => $custom_field) {
            // just import interfused fields
...
add_action('pm_one_click_new_menu_item', 'pm_one_click_import_add_metadata_menu_item', 10, 3);


//  filter out all revslider and layerslider images from export
function pm_one_click_export_main_content_attachments( $attachments ) {
    $slideshows = apply_filters('pm_one_click_export_slideshows', 'import.json');
    $ignore_files = array();
    foreach ($slideshows as $slideshow) {
        // get the zip file
        $zip = new ZipArchive();
        $zip->open(PM_THEME_DIR . $slideshow['filename']);
...
add_filter( 'pm_one_click_export_main_content_attachments', 'pm_one_click_export_main_content_attachments', 10, 1 );
 ...
 **/
function pm_one_click_checklist()
{
    // get packages so we can get url to test
    $packages = pm_filter_import_packages(array());
    ...
    return array(
        ...
        array(
            ...
            'args' => array(
                ...
                'value' => 'ZipArchive',
                'ok_message' => __('Your server has PHP Zip or unzip_file enabled. Revolution Slider import will work.', 'my_theme-admin-td'),
                'fail_message' => __('Your server does not have PHP Zip enabled or unzip_file function - Revolution Slider slides will not be able to be unpacked. Contact your hosting provider.', 'my_theme-admin-td')
            )
        )
    );
}
add_filter('pm_one_click_checklist', 'pm_one_click_checklist', 10, 1);
...
add_filter('pm_one_click_details', 'pm_one_click_details', 10, 1);
 ...
 * @author
 **/
function pm_one_click_final_setup($data, $OneClick)
{
    global $pm_theme;
        ...
        foreach ($data['theme_options'] as $id => $value) {
            ...
            switch($id) {
                ...
                case 'staff_archive_page':
                    ...
                    break;
                case 'site_stack':
                    $new_id = $OneClick->install_package->lookup_map('pm_stack', $value);
                    if (false !== $new_id) {
                        $new_value = $new_id;
                    }
                    // save new css to file
                    if (!class_exists('InterfusedStacks')) {
                        require_once(PM_STACKS_DIR . 'inc/InterfusedStacks.php');
                    }
                    // get stack instance and save the meta data to the file
                    $OxyStack = InterfusedStacks::instance();
            ...
            if (null !== $new_value) {
                $pm_theme->set_option($id, $new_value);
            }
        }
    }
}
add_action('pm_one_click_final_setup', 'pm_one_click_final_setup', 10, 2);
