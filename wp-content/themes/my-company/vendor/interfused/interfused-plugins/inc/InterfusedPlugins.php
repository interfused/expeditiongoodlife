...
require_once(PM_PLUGINS_INSTALLER_DIR . 'inc/InterfusedPluginsTable.php');

class InterfusedPlugins
{
    ...
    public function render_plugins_page()
    {
        ...
        if (isset($_GET['action']) && 'install-plugin' === $_GET['action']) {
            ...
            $plugins_table = new InterfusedPluginsTable($plugin_options);
            ...
            // create plugins page
            ob_start();
            include(PM_PLUGINS_INSTALLER_DIR . 'partials/plugins.php');
    ...
    private function save_purchase_code()
    {
        if (isset($_POST['save-purchase-code'])) {
            ...
            $this->add_message(__('Purchase code updated', 'my_theme-admin-td'), 'updated');
        }
    }
    ...
    private function plugin_install()
    {
        ...
        if (isset($_GET['id'] ) && (isset($_GET['action'] ) && 'install-plugin' === $_GET['action'])) {
            check_admin_referer('pm-plugin-install');
            ...
            switch($plugin['type']) {
                case 'wordpress':
                    ...
                    $title = sprintf(__('Installing Plugin: %s', 'my_theme-admin-td'), $api->name . ' ' . $api->version);
                ...
                case 'premium':
                    if (! current_user_can('upload_plugins')) {
                        wp_die(__('You do not have sufficient permissions to install plugins on this site.', 'my_theme-admin-td'));
                    }

                    $upgrader = new Plugin_Upgrader(new Plugin_Installer_Skin(compact('type', 'title', 'nonce', 'url')));
                    $plugin_options = $this->get_options();
                    $key = base64_encode($plugin_options['purchase-code']);
                    $download_link = 'http://updates.interfused.com/themes/' . THEME_SHORT . '/' . $plugin['id'] . '/' . $key;
    ...
    private function plugin_activate()
    {
        if (isset($_GET['action']) && 'activate-plugin' === $_GET['action']) {
            if (!current_user_can('update_plugins')) {
                wp_die(__('You do not have sufficient permissions to update plugins for this site.', 'my_theme-admin-td'));
            }

            check_admin_referer('pm-plugin-activate');

            if (isset($_GET['path']) && is_plugin_inactive($_GET['path'])) {
                if (null === activate_plugin($_GET['path'])) {
                    $this->add_message(__('The following plugin was activated successfully:', 'my_theme-admin-td') . ' <strong>' . $_GET['plugin'] . '</strong>', 'updated');
                }
            }
        }
    }
    ...
    public function plugin_complete_actions($install_actions)
    {
        if (isset($_GET['page']) && THEME_SHORT . '-plugins' === $_GET['page']) {
            ...
            $install_actions = array(
                'plugins_page' => '<a href="' . $url . '" title="' . esc_attr__('Return to theme plugin installer', 'my_theme-admin-td') . '" target="_parent">' . __('Return to theme plugin installer', 'my_theme-admin-td') . '</a>'
            );
        }
