...
function serverValsUpdatetimedCount() {
	...
	t = setTimeout("serverValsUpdatetimedCount()", 10000);
}
		...
		success: function(data){
			...
			jQuery(res).each(function(){
			  ...
			  if ( jQuery('.tr-post-' + newPost['post_id']   ).length == 0  ){
			    ...
			    jQuery('.waiting_posts tbody').append('<tr class="tr-post tr-post-' + newPostId +' alternate"> <td class="max-1">1</td> <td><a href="' + newPost['post_uri'] + '">' + newPost['post_title'] + '</a></td><td class="max-1 max-2 ">' + newPost['pin_text'] + '</td><td  class="max-1 max-2 ">' +boards[ newPost['pin_board'] ]+ '</td><td  class="max-1 max-2 ">' + newPost['post_status'] + '</td><td  class="max-1 max-2 ">('+ newPost['pin_try'] +')  </td><td><a class="wp_pinterest_automatic_delete_post" data-post="'+newPostId+'" href="#">delete post pins</a><span class="spinner-delete-'+newPostId+' spinner"></span></td></tr>');
...
jQuery(document).ready(function() {
	...
	jQuery('#wp_pinterest_automatic_clear_queue').click(function() {
		jQuery.ajax({
            ...
            beforeSend:function(){
            	console.log('sending');
            	jQuery('.spinner-clear').show();
            },
            
            success:function(data){
            	console.log('suc');
            	jQuery('.spinner-clear').hide();
