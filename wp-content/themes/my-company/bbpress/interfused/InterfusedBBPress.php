...
define('PM_BBPRESS_DIR', PM_THEME_DIR . 'bbpress/');
define('PM_BBPRESS_URI', PM_THEME_URI . 'bbpress/');

abstract class InterfusedBBPress
{
    ...
    protected function __construct($options_file)
    {
        ...
        if (is_admin()) {
            ...
            // theme styling hooks
            add_filter('pm-bbpress-global-top', array(&$this, 'global_page_top_section'), 10, 1);
            add_filter('pm-bbpress-global-bottom', array(&$this, 'global_page_bottom_section'), 10, 1);

            // add header - higher priority means it will go above section ^
            add_filter('pm-bbpress-global-top', array(&$this, 'global_page_header'), 5, 1);
        }
    }
    ...
    {
        global $pm_theme;
        $pm_theme->register_sidebar('BBPress Sidebar', 'Used on all BBPress Pages', '', 'bbpress-sidebar');
    }
    ...
    {
        wp_enqueue_style('pm-bbpress', PM_BBPRESS_URI . 'assets/css/bbpress.css');
    }
    ...
    {
        bbp_get_template_part( '/partials/global-bbpress/global-page-top', pm_get_option('bbpress_style'));
    }

    public function global_page_bottom_section()
    {
        bbp_get_template_part('/partials/global-bbpress/global-page-bottom', pm_get_option('bbpress_style'));
    }
    ...
    {
        $args['before'] = '<div class="bbp-topic-tags"><p><small>' . esc_html__('Tagged', 'my_theme-td') . '</small>';
    ...
    protected function get_page_header_title()
    {
        ...
        if (bbp_is_forum_archive()) {
            return $pm_theme_options['bbpress_header_forums'];
        } else if (bbp_is_topic_archive()) {
            return $pm_theme_options['bbpress_header_topics'];
