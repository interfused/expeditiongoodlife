...
class InterfusedMediaUpload
{
    public function __construct()
    {
        if (isset($_REQUEST['pm_upload_to_option'])) {
            // remove url tab
            ...
            wp_enqueue_script('theme-options-upload', PM_TF_URI . 'assets/javascripts/theme-options-upload.js');
        }
    }

    public function upload_form_extra_parameter($form_action_url, $type)
    {
        $form_action_url = $form_action_url . '&pm_upload_to_option=true&amp;input_id=' . $_GET['input_id'];
    ...
    public function image_attachments($form_fields, $post)
    {
        ...
        $send = '<input type="submit" onclick="oxyThemeOptions.mediaOptionSelectImage(\'' . $_REQUEST['input_id'] . '\',\'' . $post->guid . '\',\'' . $post->ID . '\')" class="button" value="' .__('Select Image', 'my_theme-admin-td') . '">';
