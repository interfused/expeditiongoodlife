 ...
 * @version 1.10.3
 */
if (!class_exists('InterfusedOptions')) {

    /**
     * Main theme options class
     *
     **/
    class InterfusedOptions
    {
        /**
        ...
        public function admin_bar_menu($wp_admin_bar)
        {
            ...
            foreach ($this->option_pages as $option_page) {
                if ($option_page['main_menu'] == true) {
                    $wp_admin_bar->add_node(array('id' => THEME_SHORT , 'title' => THEME_NAME . ' ' . __('Theme', 'my_theme-admin-td') , 'href' => admin_url('admin.php?page=' . $option_page['slug'])));
                    break;
                }
            }
        ...
        public function validate_options($new_options)
        {
           // reset button was not pressed , so we validate and update the options
            ...
            // for the import of the options
                            ...
                            if (isset($new_options[$field['id']])) {
                                ...
                                if (isset($field['validation'])) {
                                    ...
                                    foreach ($validators as $validation) {
                                        ...
                                        $class_file = PM_TF_DIR . 'options/validation/Interfused' . ucfirst($validation) . '.php';
                ...
                $customise_options = apply_filters('pm-customise-fields', array());
            ...
            if (isset($_POST['option_page'])) {
                do_action('pm-validate-options-' . $_POST['option_page']);
            }
        ...
        public function create_default_options()
        {
            ...
            $customiser_options = apply_filters('pm-customise-fields', array());
        ...
        public function option_page_loaded()
        {
            if (isset($_GET['settings-updated']) && $_GET['settings-updated'] == true) {
                do_action('pm-options-updated-' . $_GET['page']);
            }
        }
        ...
        public function enqueue_scripts($hook)
        {
            ...
            if (isset($this->hooks[$hook])) {
                ...
                wp_enqueue_style('pm-option-page');

                wp_enqueue_style('jquery-interfused-ui-theme');
                wp_enqueue_script('theme-options-page', PM_TF_URI . 'assets/javascripts/theme-options-page.js', array('jquery', 'jquery-ui-tooltip'));

                // now load any option specific js / css
                foreach ($this->hooks[$hook]['sections'] as $section) {
                    foreach ($section['fields'] as $field) {
                        if (isset($field['type'])) {
                            $new_field = InterfusedOptions::create_option($field);
                            if ($new_field !== false) {
                                $new_field->enqueue();
                            }
                        }
        ...
        { ?>
            ...
            <div class="wrap interfused-options-page">
                <div class="icon32">
                    <img src="<?php echo PM_TF_URI . '../../../interfused/interfused-framework/inc/images/interfused.png' ?>" alt="Interfused logo">
                </div>
                ...
                <form method="post" action="options.php">
                    ...
                    <div class="submit-footer">
                        <?php submit_button(__('Save Changes', 'my_theme-admin-td'), 'primary', 'save_changes'); ?>
                        <?php submit_button(__('Restore Defaults', 'my_theme-admin-td'), 'secondary', 'reset_options'); ?>
                    </div>
                </form>
            </div>
        ...
        public function render_option($field)
        {
            ...
            // create new field
            $form_field = InterfusedOptions::create_option($field, $value, $attr);
            $form_field->render();

            if (isset($field['desc'])) {
                echo '</td><td>';
                echo '<a class="description" title="' . $field['desc'] . '"><img src="' . PM_TF_URI . 'assets/images/what.png" alt="Tooltip"></a>';
            }
        }
        ...
        public static function create_option($field, $value = '', $attr = array())
        {
            if (isset($field['type'])) {
                ...
                $class_file = isset($field['class-file']) ? $field['class-file'] : PM_TF_DIR . 'inc/options/fields/' . $field['type'] . '/Interfused' . ucfirst($field['type']) . '.php';
                if (file_exists($class_file)) {
                    require_once $class_file;
