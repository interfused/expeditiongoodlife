...
define('PM_ONECLICK_DIR', PM_THEME_DIR . 'vendor/interfused/interfused-one-click/');
define('PM_ONECLICK_URI', PM_THEME_URI . 'vendor/interfused/interfused-one-click/');

define('PM_IMPORT_OK', 1);
define('PM_IMPORT_FAIL', 0);
define('PM_IMPORT_EXISTS', -1);

require_once PM_ONECLICK_DIR . 'inc/InterfusedPackageInstall.php';

class InterfusedOneClick
{
    ...
    {
        $this->meta_id_key = 'pm-' . THEME_SHORT . '-original-id';

        add_action('init', array(&$this, 'init'));
        // register hook for before option page
        add_action(THEME_SHORT . '-oneclick-before-page', array(&$this, 'render'));
        // add ajax calls
        add_action('wp_ajax_pm_import_package_details', array(&$this, 'render_packages_detail_popup'));

        add_action('wp_ajax_pm_import_start', array(&$this, 'import_start'));

        add_action('wp_ajax_pm_import_posts', array(&$this, 'import_posts'));

        add_action('wp_ajax_pm_import_menu', array(&$this, 'import_menu'));

        add_action('wp_ajax_pm_import_final_setup', array(&$this, 'final_setup'));

        add_action('wp_ajax_pm_import_slideshow', array(&$this, 'import_slideshow'));
    }

    public function init()
    {
        include PM_ONECLICK_DIR . 'inc/option-pages.php';
    }
    ...
    public function import_start()
    {
        ...
        $return->data = __('Could not validate nonce', 'my_theme-admin-td');

        if (isset($_POST['nonce'])) {
            if (wp_verify_nonce($_POST['nonce'], 'pm-importer')) {
                $this->install_package = InterfusedPackageInstall::instance($_POST['installPackageId']);
    ...
    public function import_posts()
    {
        ...
        if (isset($_POST['nonce']) && wp_verify_nonce($_POST['nonce'], 'pm-importer')) {
            ...
            if (is_array($posts) && count($posts) > 0) {
                ...
                $return->status = true;
                $return->data = __('Import Posts OK', 'my_theme-admin-td');
            }
        }
    ...
    public function import_post($post)
    {
        ...
        $this->install_package->add_log_message(__('Starting import of "', 'my_theme-admin-td') . $post['post_title'] . '"');
        ...
        if (!$this->post_already_installed($post)) {
            if ($post) {
                ...
                $post = apply_filters('pm_one_click_before_insert_post', $post, $this);
            ...
            if ($post_return_id !== null) {
                ...
                $post = apply_filters('pm_one_click_after_insert_post', $post, $this);

                $this->install_package->set_log_status(PM_IMPORT_OK);
                $this->install_package->add_log_message(__('Post Created.', 'my_theme-admin-td'));
            } else {
                $this->install_package->set_log_status(PM_IMPORT_FAIL);
                $this->install_package->add_log_message(__('Failed to create post.', 'my_theme-admin-td'));
            }
        } else {
            $this->install_package->set_log_status(PM_IMPORT_EXISTS);
            $this->install_package->add_log_message(__('Import stopped reason - Post already exists.', 'my_theme-admin-td'));
        }
    ...
    public function create_post($post)
    {
        ...
        if ($new_id !== 0) {
            ...
            $this->install_package->add_log_message(__('Import Post OK', 'my_theme-admin-td'));
            ...
            if (isset($post['custom_fields'])) {
                ...
                $this->install_package->add_log_message(__('Added ' . count($post['custom_fields']) . ' custom fields', 'my_theme-admin-td'));
            }
            ...
            if (isset($post['taxonomies'])) {
                ...
                $this->install_package->add_log_message(__('Added ' . count($taxonomies) . ' taxonomies', 'my_theme-admin-td'));
            }

            // handle post_format
            if (isset($post['format']) && $post['format'] !== false) {
                $this->install_package->add_log_message(__('Set post format ' . $post['format'], 'my_theme-admin-td'));
                set_post_format($new_id, $post['format']);
            }

            if (isset($post['attachments'])) {
                foreach ($post['attachments'] as $old_attachment_id) {
                    $this->install_package->add_log_message(__('Set attachment', 'my_theme-admin-td'));
    ...
    public function create_attachment($post)
    {
        ...
        if (is_wp_error($response)) {
            $this->install_package->add_log_message(__('Could not download - ' . $image_url, 'my_theme-admin-td'));
        } else {
            $image_body = wp_remote_retrieve_body($response);
            $this->install_package->add_log_message(__('Downloaded image data - ' . $image_url, 'my_theme-admin-td'));
            ...
            if ($upload['error']) {
                $this->install_package->add_log_message(__('Could not upload to WordPress - ' . $upload['error'], 'my_theme-admin-td'));
            }
            ...
            if (0 !== $attach_id) {
                ...
                $this->install_package->add_log_message(__('Attachment created', 'my_theme-admin-td'));

                // regenerate thumbnails
                $attach_data = wp_generate_attachment_metadata($attach_id, $upload['file']);
                wp_update_attachment_metadata($attach_id, $attach_data);

                $this->install_package->add_log_message(__('Generated Thumbnails', 'my_theme-admin-td'));
    ...
    public function import_menu()
    {
        ...
        if (isset($_POST['nonce']) && wp_verify_nonce($_POST['nonce'], 'pm-importer')) {
            ...
            $this->install_package->add_log_message(__('Started Import Menu.', 'my_theme-admin-td'));
            ...
            if (false === $already_exists) {
                ...
                $menu = apply_filters('pm_one_click_before_wp_create_nav_menu', $menu, $this);
                ...
                if (!is_wp_error($new_menu_id)) {
                    ...
                    $this->install_package->set_log_status(PM_IMPORT_OK);
                    $this->install_package->add_log_message(__('Menu Created.', 'my_theme-admin-td'));
                    ...
                    $this->install_package->set_log_status(PM_IMPORT_FAIL);
                    $this->install_package->add_log_message(__('Call to wp_create_nav_menu failed.', 'my_theme-admin-td'));
                }
            } else {
                $return->status = true;
                $return->data = 'Menu exists';
                $this->install_package->set_log_status(PM_IMPORT_EXISTS);
                $this->install_package->add_log_message(__('Import stopped reason - Menu already exists.', 'my_theme-admin-td'));
            }
    ...
    private function import_menu_item($menu_item, $menu_id)
    {
        ...
        $new_menu_item = apply_filters('pm_one_click_before_wp_update_nav_menu_item', $new_menu_item, $menu_item, $this);
        ...
        if (!is_wp_error($new_menu_item_id)) {
            ...
            do_action('pm_one_click_new_menu_item', $new_menu_item_id, $menu_item, $this);
            // add to map
    ...
    public function final_setup()
    {
        ...
        if (isset($_POST['nonce']) && wp_verify_nonce($_POST['nonce'], 'pm-importer') && isset($_POST['data'])) {
            ...
            do_action('pm_one_click_final_setup', $data, $this);
            $this->install_package->set_log_status(PM_IMPORT_OK);
            $this->install_package->close_item_log();
    ...
    public function import_slideshow()
    {
        ...
        if (isset($_POST['nonce']) && wp_verify_nonce($_POST['nonce'], 'pm-importer')) {
            ...
            if (!$this->slideshow_already_installed($slideshow)) {
                        ...
                        switch($slideshow['type']) {
                            ...
                            case 'revslider':
                                if (is_plugin_active('revslider/revslider.php')) {
                                    ...
                                    $this->install_package->set_log_status(PM_IMPORT_OK);
                                    $this->install_package->add_log_message(__('Revslider Created.', 'my_theme-admin-td'));
                                    $return->status = true;
                                }
                                break;
                        }
                ...
                $this->install_package->set_log_status(PM_IMPORT_EXISTS);
                $this->install_package->add_log_message(__('Import stopped reason - Slideshow already exists.', 'my_theme-admin-td'));
            }
        }
    ...
    {
        $this->install_package = InterfusedPackageInstall::instance($_POST['one_click_package_id']);
        $current_package = $this->install_package->get_install_package();

        $theme_check_list = apply_filters('pm_one_click_checklist', array());
        $checks = array();
        $check_list_ok = true;
        ...
        foreach ($theme_check_list as $check_item) {
            ...
            $class_file = PM_ONECLICK_DIR . 'checks/Interfused' . $check_item['name'] . '.php';
            ...
            // lets make a check
            $check_class = 'Interfused' . $check_item['name'];
        ...
        include(PM_ONECLICK_DIR . 'partials/checklist.php');
    ...
    private function render_packages_page()
    {
        ...
        $packages = apply_filters('pm_one_click_import_packages', array());
        ...
        include(PM_ONECLICK_DIR . 'partials/packages.php');
        include( ABSPATH . 'wp-admin/admin-footer.php' );
        die();
    }
    ...
    {
        $this->install_package = InterfusedPackageInstall::instance($_GET['id']);
        $current_package = $this->install_package->get_install_package();
        $installer_details = apply_filters('pm_one_click_details', array());
        ...
        include(PM_ONECLICK_DIR . 'partials/package-detail.php');
        die();
    }
    ...
    public function render_install_page()
    {
        $this->install_package = InterfusedPackageInstall::instance($_POST['one_click_package_id']);
        $current_package = $this->install_package->get_install_package();
        include(PM_ONECLICK_DIR . 'partials/install.php');
        include( ABSPATH . 'wp-admin/admin-footer.php' );
        die();
    }
    ...
    public function render_finished_page()
    {
        ...
        include(PM_ONECLICK_DIR . 'partials/install-complete.php');
        include( ABSPATH . 'wp-admin/admin-footer.php' );
        die();
    }
    ...
    public function render_remove_page()
    {
        ...
        include(PM_ONECLICK_DIR . 'partials/remove.php');
        include( ABSPATH . 'wp-admin/admin-footer.php' );
        die();
    }
    ...
    {
        $this->install_package = InterfusedPackageInstall::instance($_POST['removePackageId']);
        $remove_package = $this->install_package->get_install_package();
        $installed_items = $this->install_package->get_installed_items();
        delete_option($remove_package['id']);
        include(PM_ONECLICK_DIR . 'partials/remove-content.php');
        include( ABSPATH . 'wp-admin/admin-footer.php' );
        die();
    }
}

$one_click_install = new InterfusedOneClick();
