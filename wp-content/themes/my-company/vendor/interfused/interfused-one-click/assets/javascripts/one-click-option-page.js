...
(function( $ ){
    $(document).ready(function($){
        ...
        function installPackage() {
            if( installPackages.length > 0 ) {
                ...
                switch( currentPackage.type ) {
                    case 'interfused':
                        startInterfusedPackage( currentPackage );
                        break;
                }
            }
            else {
        ...
        function startInterfusedPackage( interfusedPackage ) {
            $.getJSON( importInfo.themeURL + 'import/' + interfusedPackage.filename, function( importJSONData ) {
                json = importJSONData;
                // tell wp we are starting an import
                $.post( importInfo.ajaxURL, {
                    action: 'pm_import_start',
                    nonce: importInfo.importNonce,
                    installPackage: interfusedPackage
                }).
                    ...
                    if( response.status ) {
                        ...
                        initProgressBar( interfusedPackage.filename, dataLength );
