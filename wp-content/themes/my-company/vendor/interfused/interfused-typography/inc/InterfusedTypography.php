...
class InterfusedTypography
{
    ...
    public function __construct()
    {
        ...
        // get font info
        add_action('wp_ajax_pm_get_font', array(&$this, 'get_font'));
    }

    public function init()
    {
        wp_register_script('pm-typography-select2', PM_TYPOGRAPHY_URI . 'assets/javascripts/select2.js', array('jquery'));
        wp_register_style('pm-typography-select2', PM_TYPOGRAPHY_URI . 'assets/css/select2.css');
    }

    public function admin_init()
    {
        // check if old plugin is on
        $type_plugin_url = 'interfused-type/interfused-type.php';
    ...
    { ?>
        <div class="error">
            <p><?php _e('Looks like you have the <strong>Interfused Typography Plugin</strong> Installed - this is no longer needed please delete it <a href="' . admin_url('plugins.php') . '">here on your plugins page</a>', 'my_theme-admin-td'); ?></p>
        </div>
    <?php
    }
    ...
    {
        return include PM_TYPOGRAPHY_DIR . 'inc/providers/system.php';
    }

    public function get_google_fonts()
    {
        return get_option('pm-google-fonts');
    }

    public function get_typekit_fonts()
    {
        return get_option('pm-typekit-fonts');
    }
    ...
    public function google_fetch_fonts_list()
    {
        ...
        if (isset($_POST['nonce'])) {
            if (wp_verify_nonce($_POST['nonce'], 'google-fetch-fonts-nonce')) {
                ...
                $resp->message = __('Could not verify nonce', 'my_theme-admin-td');
            }
        }
    ...
    public function google_update_font_list()
    {
        ...
        if (is_wp_error($response)) {
            $resp->status = false;
            $resp->message = __('Error fetching Google Font List', 'my_theme-admin-td');
        } else {
            // we got a new list , so we update the theme options
            $list = json_decode($response, true);
            update_option('pm-google-fonts', $list['items']);
            $resp->status = true;
        }
            ...
            if (wp_verify_nonce($_POST['nonce'], 'typekit-kits-nonce')) {
                ...
                if (null !== $kits) {
                    if (empty($kits->errors)) {
                        ...
                        update_option('pm-typekit-fonts', $kits_details);
                        $resp->status = true;
                    }
                }
            } else {
    ...
    public function fontstack_list()
    {
        ...
        if (isset($_POST['nonce'])) {
            if (wp_verify_nonce($_POST['nonce'], 'list-fontstack')) {
                ...
                $response->data = get_option('pm-fontstack');
    ...
    public function font_modal()
    {
        if (isset($_GET['nonce'])) {
            if (wp_verify_nonce($_GET['nonce'], 'font-modal')) {
                ...
                $save_button_text = $isNew ? __('Add font', 'my_theme-admin-td') : __('Update Font', 'my_theme-admin-td');

                // get font info for font options
                $font_info = $this->fetch_font($_GET['family'], $_GET['provider']);

                $default_font_css = apply_filters('pm_default_typography_css', '');
                ...
                $elements = array(
                    'body'       => __('All (body tag)', 'my_theme-admin-td'),
                    'headings'   => __('Headings (h1-h6 tags)', 'my_theme-admin-td'),
                    'forms'      => __('Forms (all input tags)', 'my_theme-admin-td'),
                    'blockquote' => __('Block Quote (blockquote tag)', 'my_theme-admin-td'),
                );
                ...
                wp_enqueue_script('add-font', PM_TYPOGRAPHY_URI . 'assets/javascripts/add-font.js', array('jquery'));
                wp_enqueue_style('add-font', PM_TYPOGRAPHY_URI . 'assets/css/add-font.css');
                ...
                wp_localize_script('add-font', 'localData', array(
                    ...
                    'nonce'     => wp_create_nonce('interfused-add-fontstack'),
                ));

                ob_start();
                include(PM_TYPOGRAPHY_DIR . 'partials/font-modal.php');
        ...
        if (isset($_POST['nonce'])) {
            if (wp_verify_nonce($_POST['nonce'], 'update-fontstack')) {
                if (!empty($_POST['fontstack'])) {
                    ...
                    update_option('pm-fontstack', $fontstack);
                    // now update the font css
                    $this->create_font_css($fontstack);
                    // now update the font js
                    $js = $this->create_font_js($fontstack);
                    update_option('pm-typography-js', $js);
                } else {
                    delete_option('pm-fontstack');
                    delete_option('pm-typography-css');
                    delete_option('pm-typography-js');
                    $fontstack = array();
                }
                // set response to ok
                $resp->status = true;
                $resp->message = __('Fontsack Updated Successfully', 'my_theme-admin-td');
    ...
    public function create_font_css($fontstack)
    {
        ...
        update_option('pm-typography-css', $css);
    }
...
JS;
    ...
    public function pm_get_font_weight_style($variant, $provider)
    {
        $variations = array(
            'font-style' => array(
                'n' => 'normal',
    ...
    public function render_typography_page()
    {
        add_thickbox();
        ob_start();
        include(PM_TYPOGRAPHY_DIR . 'partials/typography-page.php');
        $output = ob_get_contents();
        ob_end_clean();
        echo $output;
        die();
    ...
    {
        include(PM_TYPOGRAPHY_DIR . 'inc/options/font-select.php');
        $font_select = new InterfusedFontSelect(array(), '', array('id' => 'fontstack-select'), $this);
        $font_select->render();
    }
    ...
    public function fontstack_defaults()
    {
        ...
        if (isset($_POST['nonce'])) {
            if (wp_verify_nonce($_POST['nonce'], 'default-fonts')) {
                $google_fonts = get_option('pm-google-fonts', false);
                if (false !== $google_fonts) {
                    $get_response = wp_remote_get(PM_THEME_URI . 'import/default-fonts.php', array('sslverify' => false));
                    ...
                    $resp->status = true;
                    $resp->data = get_option('pm-fontstack');
                    $resp->message = __('Default Fonts Installed Successfully', 'my_theme-admin-td');
                } else {
                    $resp->message = __('No Google Fonts Installed. Please go to ' . THEME_NAME . ' -> Fonts and update your google fonts', 'my_theme-admin-td');
                }
            }
        }
