 ...
 */
class InterfusedSelect extends InterfusedOption
{
    /**
     * Creates option
     *
    ...
    public function load_select_data($database)
    {
        ...
        switch ($database) {
            ...
            case 'taxonomy':
                if (isset($this->_field['taxonomy'])) {
                    ...
                    switch ($this->_field['taxonomy']) {
                        ...
                        case 'posts':
                            ...
                            break;
                        case 'pm_portfolio_image':
                            $data = get_posts(array('posts_per_page'=> -1, 'post_type'=>'pm_portfolio_image'));
                            break;
                        default:
                ...
                break;
            case 'revolution':
                $this->_field['blank'] = __('Select a Slideshow', 'my_theme-admin-td');
                global $wpdb;
            ...
            case 'layerslider':
                if (class_exists('LS_Sliders')) {
                    ...
                    $this->_field['blank'] = __('Please activate LayersSlider plugin', 'my_theme-admin-td');
                }
                break;
            case 'slideshow':
                $this->_field['blank'] = __('Select a Slideshow', 'my_theme-admin-td');
                $data = get_categories(array('orderby' => 'name', 'hide_empty' => '0', 'taxonomy' => 'pm_slideshow_categories'));
                break;

            case 'get_option':
                $options = get_option(THEME_SHORT. '-options');
                if (isset($options['unregistered'][$this->_field['option']])) {
                    $data = pm_get_option($this->_field['option']);
                    $unregistered = pm_get_option('unregistered');
            ...
            case 'staff_featured':
                $this->_field['blank'] = __('Select a Staff member', 'my_theme-admin-td');
                $posts =  get_posts('showposts=-1&post_type=pm_staff');
                foreach ($posts as $staff):
                    $data[$staff->post_title] = $staff->ID;
                endforeach;
                break;
            case 'icons':
                if (file_exists(PM_THEME_DIR . 'inc/options/global-options/fontawesome.php')) {
                    $data = include PM_THEME_DIR . 'inc/options/global-options/fontawesome.php';
                }
                break;
            case 'categories':
                //$this->_field['blank'] = __('all categories', 'my_theme-admin-td');
                $data = get_categories(array('orderby' => 'name', 'hide_empty' => '0'));
                break;
            case 'portfolios':
                $this->_field['blank'] = __('Select a Portfolio', 'my_theme-admin-td');
                $data = get_categories(array('orderby' => 'name', 'hide_empty' => '0', 'taxonomy' => 'pm_portfolio_categories'));
                break;
            case 'typekit_kits':
                ...
                if (isset($theme_options['unregistered']['typekit_kits'])) {
                    ...
                    if (!empty($kits_details)) {
                        $this->_field['blank'] = __('Select a kit', 'my_theme-admin-td');
    ...
    public function enqueue()
    {
        ...
        if (isset($this->attr['class']) && strrpos($this->attr['class'], 'select2') !== false) {
            ...
            wp_enqueue_script('select2-select-option', PM_TF_URI . 'inc/options/fields/select/select2.js', array('jquery'));
        }
    }
}
