...
class InterfusedMegaMenu
{
    ...
    public function __construct()
    {
        if (is_admin()) {
            ...
            require_once PM_MEGA_MENU_DIR . 'walkers/AdminBootstrapMegaMenuWalker.php';
            ...
            add_action('wp_ajax_pm_save_menu', array(&$this, 'wp_ajax_pm_save_menu'));

            // custom hook for adding options to top of menu options
            add_action('pm_admin_menu_options_top', array(&$this, 'pm_admin_menu_options_top'), 10, 4);

            // custom hook for adding options to bottom of menu options
            add_action('pm_admin_menu_options_bottom', array(&$this, 'pm_admin_menu_options_bottom'), 10, 4);
        ...
        if (isset($locations['primary'])) {
            ...
            if ($menu_items !== false && is_array($menu_items)) {
                foreach ($menu_items as $menu_item) {
                    if ('pm_mega_columns' === $menu_item->object) {
                        $pm_widget = get_post_meta($menu_item->ID, 'pm_widget', true);
                        if ('on' === $pm_widget) {
                            register_sidebar(array(
                                'id'            => 'menu-' . $menu_item->ID,
                                'name'          => __('Menu', 'my_theme-admin-td') . ' - ' . $menu_item->title,
    ...
    public function current_screen($current_screen)
    {
        if ('nav-menus' === $current_screen->base) {
            wp_enqueue_style('pm-mega-menu', PM_MEGA_MENU_URI . 'assets/css/pm-mega-menu.css');

            if ('on' === pm_get_option('ajax_menu_save')) {
                wp_enqueue_script('pm-ajax-save-menu', PM_MEGA_MENU_URI . 'assets/js/ajax-save-menu.js', array('jquery'));
            }
    ...
    public function pm_admin_menu_options_top($item, $depth, $args, $id)
    {
        ...
        switch($item->object)
        {
            ...
            case 'pm_mega_columns':
                include(PM_MEGA_MENU_DIR . 'partials/options/top/' . $item->object . '.php');
                break;
        }
    }

    public function pm_admin_menu_options_bottom($item, $depth, $args, $id)
    {
        $item_id = esc_attr($item->ID);
        switch($item->object)
        {
        ...
        include(PM_MEGA_MENU_DIR . 'partials/options/bottom/global.php');
    }

    public function wp_setup_nav_menu_item($menu_item)
    {
        $menu_item->pm_mega_url     = get_post_meta($menu_item->ID, 'pm_mega_url', true);
        $menu_item->pm_icon         = get_post_meta($menu_item->ID, 'pm_icon', true);
        $menu_item->pm_bg_url       = get_post_meta($menu_item->ID, 'pm_bg_url', true);
        $menu_item->pm_col_url      = get_post_meta($menu_item->ID, 'pm_col_url', true);
        $menu_item->pm_special      = get_post_meta($menu_item->ID, 'pm_special', true);
        $menu_item->pm_widget       = get_post_meta($menu_item->ID, 'pm_widget', true);
        $menu_item->pm_label        = get_post_meta($menu_item->ID, 'pm_label', true);
        $menu_item->pm_label_type   = get_post_meta($menu_item->ID, 'pm_label_type', true);
        $menu_item->pm_mega_borders = get_post_meta($menu_item->ID, 'pm_mega_borders', true);
        return $menu_item;
    }
     ...
     * @author
     **/
    public function wp_ajax_pm_save_menu()
    {
        @error_reporting(0); // Don't break the JSON result

        // parse variables sent from $.serialize into an array
        $pm_menu_to_save = json_decode(stripslashes($_POST['menu']), true);

        // set request and post to parsed menu data to fake the request
        $_REQUEST = $pm_menu_to_save;
        $_POST = $pm_menu_to_save;
        ...
        if (0 == $nav_menu_selected_id) {
            ...
            if ($new_menu_title) {
                ...
                $messages[] = '<div id="message" class="error"><p>' . __('Please enter a valid menu name.', 'my_theme-admin-td') . '</p></div>';
            }
