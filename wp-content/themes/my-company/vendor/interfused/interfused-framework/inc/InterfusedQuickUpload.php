...
class InterfusedQuickUpload
{
    ...
    public function __construct()
    {
        if (is_admin()) {
            ...
            $uploaders_file = PM_THEME_DIR . 'inc/quick-uploaders.php';
        ...
        if ($this->uploaders !== null && isset($_GET['page'])) {
            foreach ($this->uploaders as $post_type => $options) {
                if ($_GET['page'] == $post_type . '-quick') {
                    wp_enqueue_style('pm-quick-uploader', PM_TF_URI . 'assets/css/quick-uploader/quick-uploader.css');
                }
            }
        }
    }
    ...
    public function select_images_page($post_type, $options)
    {
        ...
        if (!current_user_can('upload_files')) {
            wp_die(__('You do not have permission to upload files.', 'my_theme-admin-td'));
        }
        ...
        if (isset($action) && $action == 'edit' && !$ID) {
            wp_die(__('You are not allowed to be here', 'my_theme-admin-td'));
        }
        ...
        $title = __('Upload New Media', 'my_theme-admin-td');
        $parent_file = 'upload.php';

        require_once '../../../interfused/interfused-framework/inc/admin-header.php';
        ...
        <?php submit_button(__('Use these images', 'my_theme-admin-td'), 'button savebutton', 'save'); ?>
        </form>
        </div>
    ...
    { ?>
        <div class="wrap pm-quick-uploader">
            <div class="icon32">
                <img src="<?php echo PM_TF_URI . '../../../interfused/interfused-framework/inc/images/interfused.png' ?>" alt="Interfused logo">
            </div>
            <h2><?php echo get_admin_page_title(); ?></h2>
            <form action="<?php echo admin_url('edit.php?post_type=' . $post_type . '&amp;page=' . $post_type . '-quick'); ?>" method="POST" accept-charset="utf-8" class="pm-options">
                <div id="poststuff">
                    <?php
                    $count = 1;
                    ...
                    <div id="post-body" class="metabox-holder columns-2">
                        <div id="post-body-content">
                            ...
                            <?php submit_button(__('Save', 'my_theme-admin-td') . ' ' .$options['item_plural']); ?>
                        </div>
                    </div>
                </div>
            </form>
    ...
    public function build($post_type, $options)
    {
        ...
        echo '<div id="message" class="updated fade"><p><strong>' . sprintf(__('Created %d %s', 'my_theme-admin-td'), count($_POST['media_attachment_id']), $options['item_plural']) . '</strong></p></div>';
    }
}

$quick_uploader = new InterfusedQuickUpload();
