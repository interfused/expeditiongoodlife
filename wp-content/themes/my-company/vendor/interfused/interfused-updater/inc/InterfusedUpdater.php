...
class InterfusedUpdater
{
    ...
    public function init()
    {
        if (!class_exists('Envato_Protected_API')) {
            ...
            require_once(PM_THEME_DIR . 'vendor/primozcigler/envato-wordpress-theme-updater/class-envato-protected-api.php');
        }
        require_once(PM_UPDATER_DIR . 'inc/InterfusedThemeUpdater.php');
        ...
        if (!empty($options['username']) && !empty($options['api']) && 'authenticated' === $options['status']) {
            ...
            InterfusedThemeUpdater::init($options['username'], $options['api'], 'Interfused.com');
        }
    }
    ...
    public function render_updater_page()
    {
        ...
        $message = __('Please insert your Envato User Name and API Key in the above section in order to receive automatic updates', 'my_theme-admin-td');
        ...
        if (isset($options['status'])) {
            switch($options['status']) {
                case 'missing':
                    ...
                    break;
                case 'invalid':
                    $message = '<span class="updater-warning">' . __('Your Envato User Name and/or API Key are wrong.', 'my_theme-admin-td') . '</span>';
                    $message .= __('Please insert the correct Envato User Name and API Key in the above section in order to receive automatic updates', 'my_theme-admin-td');
                    break;
                case 'authenticated':
                    if (isset($this->theme_status['new_version'])) {
                        $update_available = true;
                        $form_action = 'update-core.php?action=do-theme-upgrade';
                        $message = sprintf(__('Version %1$s of %2$s is available! You are currently running version %3$s', 'my_theme-admin-td'), $this->theme_status['new_version'], THEME_NAME, $this->theme_status['version']);
                    } else {
                        $message = sprintf(__('You are running the latest version of %1$s (%2$s). Your theme is up to date!', 'my_theme-admin-td'), THEME_NAME, $this->theme_status['version']);
                    }
                    break;
            }
        }
        ...
        ob_start();
        include(PM_UPDATER_DIR . 'partials/updater-page.php');
    ...
    {
        wp_enqueue_style('update-notice-style', PM_UPDATER_URI . 'assets/stylesheets/dismissable-notice.css');
        wp_enqueue_script('update-notice-script', PM_UPDATER_URI . 'assets/javascripts/dismissable-notice.js', array('jquery'));
