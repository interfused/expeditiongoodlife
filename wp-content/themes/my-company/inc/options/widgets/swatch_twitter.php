...
class Swatch_twitter extends InterfusedWidget {
    /**
    * Register widget with WordPress.
    */
    public function __construct() {
        $widget_options = array( 'description' => __( 'Displays you latest tweets', 'my_theme-admin-td') );
        parent::__construct( 'swatch_twitter-options.php', false, $name = THEME_NAME . ' - ' . __('Twitter Feed', 'my_theme-admin-td'), $widget_options );
    }
    ...
    function widget( $args, $instance ) {
        ...
        if ( empty( $account ) ) {
            if ( current_user_can('edit_theme_options') ) {
                ...
                echo '<p>' . sprintf( __( 'Please configure your Twitter username for the <a href="%s">Twitter Widget</a>.', 'my_theme-td' ), admin_url( 'widgets.php' ) ) . '</p>';
        ...
        $title               = $this->get_option( 'title', $instance, 'Twitter feed');
        ...
        if ( ! is_wp_error( $tweets ) && is_array( $tweets ) && isset( $tweets[0]->user->id ) ) {
            ...
            $this->display_tweets( $show, $title, $tweets, $hidepublicized, $before_tweet, $before_timesince, $account );
            if ( $follow_button == 'on' )
    ...
    function display_tweets( $show, $title, $tweets, $hidepublicized, $before_tweet, $before_timesince, $account ) {
        $tweets_out = 0; ?>
        <div class="sidebar-widget  widget_pm_twitter">
