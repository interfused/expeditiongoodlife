...
EasyAffiliateLinks.init = function($) {
    ...
    $('.eafl_use_link').on('change', function() {
        ...
        if(text != null && text !== 'eafl_select_link_text') {
            ...
            btn.find('option:eq(0)').prop('selected', true);
        }
    });

    $('.eafl_add_link').on('click', function() {
        var btn = $(this),
            select = btn.parents('tr').find('.eafl_use_link'),
            text = select.val();

        if(text != null && text !== 'eafl_select_link_text') {
            parent.EasyAffiliateLinks.addShortcode(select.data('id'), select.data('name'), text);
            parent.tb_remove();
        }
    });
};

EasyAffiliateLinks.resetClicks = function(id) {
    if(confirm(eafl_admin.confirm_reset_clicks)) {
        var data = {
            action: 'reset_clicks',
        ...
        jQuery.post(eafl_admin.ajax_url, data, function(out) {
            jQuery('tr#post-' + id)
                .find('.column-eafl_clicks')
                .find('.eafl_clicks').text('0')
                .siblings('.eafl_clicks_lifetime').text('0');
        });
    }
};
