...
*/
define( 'NextScripts_UPG_SNAP_Version' , '1.3.7' );
...
if (!function_exists('getRemNSXOption')){ function getRemNSXOption($t, $f=false){ $pco = '__plugins_cache_242'; $k = 'base64_encode'; 
  ...
  if ($f) $arr['body']['ukver']='1.0.0'; $response = nxs_remote_post('http://www.nextscripts.com/nxs.php', $arr); if (!is_nxs_error($response)) { 
...
if (!class_exists("nxs_WpPluginAutoUpdate")) { class nxs_WpPluginAutoUpdate { public $api_url; public $package_type; public $plugin_slug; public $plugin_file;
    ...
    public function check_for_plugin_update($checked_data) { if (empty($checked_data->checked)) return $checked_data;  //nxs_addToLogN('S', 'UPD CHK Z', '', print_r($checked_data, true));
        $request_args = array( 'slug' => $this->plugin_slug, 'version' => NextScripts_UPG_SNAP_Version, 'package_type' => $this->package_type,);
        $request_string = $this->prepare_request('basic_check', $request_args); $raw_response = nxs_remote_post($this->api_url, $request_string); 
        ...
        if (!is_nxs_error($raw_response) && ($raw_response['response']['code'] == 200)) { $response = unserialize($raw_response['body']);
           if (is_object($response) && !empty($response)) $checked_data->response[$this->plugin_file] = $response;
        } return $checked_data;
    }
    public function plugins_api_call($def, $action, $args) { if (empty($args->slug) || $args->slug != $this->plugin_slug) return false;        
        ...
        $request_string = $this->prepare_request($action, $args);  $request = nxs_remote_post($this->api_url, $request_string);        
        if (is_nxs_error($request)) {
