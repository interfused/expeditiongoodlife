...
<div class="wrap">
	...
	foreach ( $res as $rec ) {
		...
		echo '<tr class="' . $alternate . ' ' . $rec->action . '"><td data-row-id="' . $rec->id . '" class="column-date row-id">' . $i . '</td><td  class="column-response" style="padding:5px">' . urldecode ( $rec->date ) . '</td><td  class="column-response" style="padding:5px">' . $action . '</td><td  style="padding:5px">' . urldecode ( $data ) . ' </td></tr>';
		$i ++;
	}
